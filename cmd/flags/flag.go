...
type Config struct {
	...
	ServerName           string  `json:"server_name" env:"AGENT_SERVER_NAME"`                         // 首次自动发现时显示的服务器名称
	...
	EnableGPU            bool    `json:"enable_gpu" env:"AGENT_ENABLE_GPU"`                           // 启用基础 GPU 监控
	DetailedGPU          bool    `json:"detailed_gpu" env:"AGENT_DETAILED_GPU"`                       // 启用详细 GPU 监控
	LegacyDetailedGPU    bool    `json:"-"`                                                           // --gpu compatibility alias
	DisableRemoteControl bool    `json:"disable_remote_control" env:"AGENT_DISABLE_REMOTE_CONTROL"`   // 禁用远程控制
	...
	ReplayStateFile      string  `json:"replay_state_file" env:"AGENT_REPLAY_STATE_FILE"`             // 已执行指令 nonce 记录路径，防重放
	CredentialsFile      string  `json:"credentials_file" env:"AGENT_CREDENTIALS_FILE"`               // 注册凭据持久化路径
